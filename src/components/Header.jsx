...
export default function Header() {
  return (
    <motion.header 
      ...
      className="flex items-center justify-between w-full py-6 relative z-20"
    >
      {/* Left: Logo */}
      <div className="flex items-center space-x-3 cursor-pointer group">
        ...
        <div className="flex flex-col">
          <span className="text-2xl font-serif tracking-widest text-[#B49162]">AI IDEATE</span>
          <span className="text-[10px] text-gray-500 font-medium tracking-wide">Where Human & Artificial<br/>Intelligence Co-Evolve</span>
        </div>
      </div>
      ...
      <nav className="flex items-center space-x-8">
        {navItems.map((item, idx) => (
          <div key={idx} className="relative flex flex-col items-center group cursor-pointer">
            <span className={`text-sm font-semibold tracking-wide transition-colors duration-300 ${item.isComingSoon ? 'text-gray-400 group-hover:text-gray-600' : 'text-primary-blue hover:text-[#B49162]'}`}>
              {item.name}
            </span>
            {item.isComingSoon && (
              <span className="absolute -bottom-5 text-[9px] font-bold bg-[#FDF8F3] text-[#B49162] px-2 py-0.5 rounded-sm whitespace-nowrap opacity-80 group-hover:opacity-100 transition-opacity shadow-sm">
                Coming Soon
              </span>
            )}
            {!item.isComingSoon && (
      ...
      <motion.button 
        ...
        whileTap={{ scale: 0.98 }}
        className="flex items-center space-x-2 border border-[#B49162] text-[#B49162] px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-[#FDF8F3] transition-colors"
      >
